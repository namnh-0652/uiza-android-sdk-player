...
public class UZUtil {
    ...
    private static final String TAG = UZUtil.class.getSimpleName();
    ...
    public static void resizeLayout(ViewGroup viewGroup, ImageView ivVideoCover, int pixelAdded,
        ...
        if (isFullScreen) {//landscape
            widthSurfaceView = UzDisplayUtil
                    .getScreenHeightIncludeNavigationBar(viewGroup.getContext());
            heightSurfaceView = UzDisplayUtil.getScreenHeight();
        } else {//portrait
            widthSurfaceView = UzDisplayUtil.getScreenWidth();
        ...
        if (flImgThumnailPreviewSeekbar != null) {
            if (isFullScreen) {
                ...
                flImgThumnailPreviewSeekbar.getLayoutParams().height = (int) (widthSurfaceView / 4
                        * Constants.RATIO_9_16);
            } else {
                flImgThumnailPreviewSeekbar.getLayoutParams().width = widthSurfaceView / 5;
                flImgThumnailPreviewSeekbar.getLayoutParams().height = (int) (widthSurfaceView / 5
                        * Constants.RATIO_9_16);
            }
    ...
    public static void getDetailEntity(final Context context, final String entityId,
            final CallbackGetDetailEntity callback) {
        UzUtilBase.getDetailEntity(context, UZData.getInstance().getAPIVersion(), entityId,
                UZData.getInstance().getAppId(), callback);
    }

    public static void getDataFromEntityIdLive(final Context context, final String entityId,
            final CallbackGetDetailEntity callback) {
        UzUtilBase.getDataFromEntityIdLive(context, UZData.getInstance().getAPIVersion(),
                UZData.getInstance().getAppId(), entityId, callback);
    }
    ...
    private static void initEntity(Activity activity, UzVideo uzVideo, String entityId,
            boolean isLive) {
        if (activity == null) {
            throw new NullPointerException(UzException.ERR_12);
        }
        ...
        if (!UzConnectivityUtil.isConnected(activity)) {
            LLog.e(TAG, UzException.ERR_0);
            return;
        }
    ...
    private static void playCustomLinkPlay(final UzVideo uzVideo,
            final UZCustomLinkPlay uzCustomLinkPlay) {
        UZData.getInstance().setSettingPlayer(false);
        uzVideo.post(new Runnable() {
            @Override
            public void run() {
                uzVideo.initCustomLinkPlay(uzCustomLinkPlay.getLinkPlay(),
                        uzCustomLinkPlay.isLivestream());
            }
        });
    }
    ...
    public static boolean initWorkspace(Context context, int apiVersion, String domainApi,
            String token, String appId, int env, int currentPlayerId) {
        if (context == null) {
    ...
    public static void initWorkspace(Context context, int apiVersion, String domainApi,
            String token, String appId, int currentPlayerId) {
        initWorkspace(context, apiVersion, domainApi, token, appId, Constants.ENVIRONMENT_PROD,
                currentPlayerId);
    }
    ...
    public static void updateUIFocusChange(View view, boolean isFocus, int resHasFocus,
            int resNoFocus) {
        if (view == null) {
            return;
        }
    ...
    private static void communicateContext(Context context, String event) {
        if (isMiniPlayerRunning(context)) {
            ComunicateMng.MsgFromActivity msgFromActivity = new ComunicateMng.MsgFromActivity(
                    event);
    ...
    public static void saveLastServerTime(Context context, long currentTimeMillis) {
        SharedPreferenceUtil
                .put(getPrivatePreference(context), LAST_SYNCED_SERVER_TIME, currentTimeMillis);
    }

    public static long getLastServerTime(Context context) {
        return (long) SharedPreferenceUtil
                .get(getPrivatePreference(context), LAST_SYNCED_SERVER_TIME,
